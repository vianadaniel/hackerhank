...
function mostBooked2(n: number, meetings: number[][]): number {
    ...
    for (let i = 0; i < meetings.length; i += 1) {
      ...
      for (let j = 0; j < n; j += 1) {
        const roomOpen = endTimes[j] <= start;
      ...
      if (!booked) {
        ...
        endTimes[nextRoom] = endTimes[nextRoom] + meetingDuration;
        roomUses[nextRoom] += 1;
      }
    }

    const max = { room: undefined, uses: 0 };

    for (let i = 0; i < n; i += 1) {
      const currentUses = roomUses[i];

      if (currentUses > max.uses) {
        max.uses = currentUses;
        max.room = i;
      }
    }
...
function mostBooked(n: number, meetings: number[][]): number {
    const jak: { bookings: number; endTime: number }[] = Array.from({ length: n }, () => ({ bookings: 0, endTime: 0 }));
     meetings.sort((a, b) => a[0] - b[0]);
     ...
     for (let i = 0; i < meetings.length; i++) {
         ...
         if (!booked) {
             let jakA = jak.reduce((min, room, i) => room.endTime < min.endTime ? room : min);
             jakA.bookings++;
